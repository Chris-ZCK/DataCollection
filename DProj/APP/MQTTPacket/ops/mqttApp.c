...
MY_MQTT_ERR mysend_config(char *msg)
{
	...
	// 打包
	len = strlen(msg);
	//printf("mysend_config=%d\r\n",len);
	crc = ModBusCRC((uint8_t *)msg, len);
	offset = mymes_serialize(MQTT_DATA_buf, len, crc);
	//strcat(MQTT_DATA_buf, msg);
	while(i<len)
	{
		MQTT_DATA_buf[offset+i]= msg[i];
		i++;
	}
	MQTT_DATA_buf[offset+i]=0;
	// printf("%d,%d####\r\n",len+offset,strlen(MQTT_DATA_buf));
	// 发送数据
	LED_GREEN_ON();
	mqtt_publish_data(MY_TOPIC_CONFIG, MQTT_DATA_buf, len+offset);
	LED_GREEN_OFF();
	return MY_MQTT_ERR_NONE;
}
