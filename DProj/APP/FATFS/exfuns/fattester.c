...
u8 act_send_picture(void)
{
	...
	mf_scan_files((u8*)SOURCE_OF_PICTURE_SD_PATH);  // 扫描文件夹
	mf_send_pics((u8*)SOURCE_OF_PICTURE_SD_PATH,(u8*)"0:ARCH",1,0);  // 发送图片
	printf("[INFO]mf_scan_files-A\r\n");
	mf_scan_files((u8*)SOURCE_OF_PICTURE_SD_PATH);  // 扫描文件架
	return 1;
}
...
u8 act_send_picture_wifi(void)
{
	...
	mf_scan_files((u8*)SOURCE_OF_PICTURE_WIFI_PATH);  // 扫描文件夹
	mf_send_pics((u8*)SOURCE_OF_PICTURE_WIFI_PATH,(u8*)"0:ARCH",1,1);  // WIFI发送图片
	printf("[INFO]mf_scan_wifi_files-A\r\n");
	mf_scan_files((u8*)SOURCE_OF_PICTURE_WIFI_PATH);  // 扫描文件架
	return 1;
}
