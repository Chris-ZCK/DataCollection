...
//	}
		IIC_MAX44009_Start();                //启动读取
		IIC_MAX44009_Send_Byte(0x94);    //从机地址（0：写）
		IIC_MAX44009_Wait_Ack();
		
		IIC_MAX44009_Send_Byte(0x03);    //发送control byte  
		IIC_MAX44009_Wait_Ack(); //等待从机回复数据
		
		IIC_MAX44009_Start();                //启动读取
		IIC_MAX44009_Send_Byte(0x95);  //从机地址（1：读），进入接收模式
		IIC_MAX44009_Wait_Ack();
		
		reg_data[0]=IIC_MAX44009_Read_Byte(0);		//读取一个字节,不继续再读,发送NAK 
		
		IIC_MAX44009_Start();                //启动读取
		IIC_MAX44009_Send_Byte(0x94);    //从机地址（0：写）
		IIC_MAX44009_Wait_Ack();
		
		IIC_MAX44009_Send_Byte(0x04);    //发送control byte  
		IIC_MAX44009_Wait_Ack(); //等待从机回复数据
		
		IIC_MAX44009_Start();                //启动读取
		IIC_MAX44009_Send_Byte(0x95);  //从机地址（1：读），进入接收模式
		IIC_MAX44009_Wait_Ack();
		reg_data[1]=IIC_MAX44009_Read_Byte(0);		//读取一个字节,不继续再读,发送NAK 
...
{
    uint8_t lux[2], lux_exponent,mantissa;

    I2C_bus_read(MAX_ADDRESS, LUX_HIGH_REG, lux, 2);

    lux_exponent    = ((lux[0] & 0xF0 ) >> 4);
    mantissa        = ((lux[0] & 0x0F ) << 4);
    mantissa       +=  (lux[1] & 0x0F );

    //lux_value    = 0.045 * ( lux_high + lux_low ) * (1<< lux_exponent);
    //*mLux_value    = 45L * ( lux[0] | lux[1] ) * (1<< lux_exponent);
	  *mLux_value    = mantissa * ( 1 << lux_exponent )*0.045;
};

int32_t FluxMeasurement_get(void)
{
	u8 i=0;
	uint32_t lus;
	while(i++<3)
